...
const NEURON_CATEGORIES = {
  ...
  // Muscles
  dorsalMuscles: neurons.filter(n => n.startsWith("MDL") || n.startsWith("MDR")),
  ventralMuscles: neurons.filter(n => n.startsWith("MVL") || n.startsWith("MVR")),
  
  // Sensory and interneurons
  sensory: neurons.filter(n => 
    n.startsWith("ADF") || n.startsWith("ASE") || n.startsWith("AWC") ||
    n.startsWith("ALM") || n.startsWith("AVM") || n.startsWith("PLM")
  ),
  interneurons: neurons.filter(n => 
    !n.startsWith("MD") && !n.startsWith("MV") && 
    !n.startsWith("VA") && !n.startsWith("DA") &&
    !n.startsWith("VB") && !n.startsWith("DB") &&
    !n.startsWith("DD") && !n.startsWith("VD") &&
    !n.startsWith("AVB") && !n.startsWith("PVC") &&
    !n.startsWith("AVA") && !n.startsWith("AVD") &&
    !n.startsWith("RIV") && !n.startsWith("SMD")
  )
} as const;
...
function hexToNormalizedFloat(hex: string, baseWeight: number): number {
  ...
  const scaledVariation = Math.sign(variation) * Math.pow(Math.abs(variation), 0.75);
...
type NeuronRole = 'forward' | 'backward' | 'turn' | 'motorA' | 'motorB' | 'motorD' | 
                  'dorsalMuscle' | 'ventralMuscle' | 'sensory' | 'interneuron';
...
function getWeightModifier(sourceRole: NeuronRole, targetRole: NeuronRole, hash: string): number {
  const hashValue = parseInt(hash.substring(0, 8), 16) / parseInt("f".repeat(8), 16);
  
  // Command neurons to motor neurons
  if ((sourceRole === 'forward' || sourceRole === 'backward') && 
      (targetRole === 'motorA' || targetRole === 'motorB')) {
    return 1.4 + (hashValue * 0.6); // 1.4 - 2.0 range for command to motor
  }
  
  // Motor neurons to muscles
  if ((sourceRole === 'motorA' || sourceRole === 'motorB' || sourceRole === 'motorD') && 
      (targetRole === 'dorsalMuscle' || targetRole === 'ventralMuscle')) {
    return 1.3 + (hashValue * 0.7); // 1.3 - 2.0 range for motor to muscle
  }
  
  // Sensory to command
  if (sourceRole === 'sensory' && 
...
async function generateDynamicWeights(transactions: any[]): Promise<WeightMap> {
  ...
  for (const tx of recentTransactions) {
    ...
    const normalizedAmount = Math.min(Math.max(Number(tx.amount) / 1e9, 0.1), 10);
    const timeSinceTransaction = Date.now() - (tx.blockTime * 1000);
    const timeMultiplier = Math.pow(Math.max(0, 1 - (timeSinceTransaction / (3600 * 1000))), 0.5);
    ...
    for (const sourceNeuron in weights) {
      ...
      for (const targetNeuron in connections) {
        ...
        const weightModifier = getWeightModifier(sourceRole, targetRole, neuronHash);
        const dynamicComponent = hexToNormalizedFloat(neuronHash, baseWeight);
        const transactionEffect = normalizedAmount * recencyMultiplier * timeMultiplier;
        ...
        weights[sourceNeuron][targetNeuron] = Math.max(-1, Math.min(4, finalWeight));
      }
    }
  }
